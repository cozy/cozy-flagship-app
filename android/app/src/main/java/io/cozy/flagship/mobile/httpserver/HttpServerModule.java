...
public class HttpServerModule extends ReactContextBaseJavaModule implements LifecycleEventListener {
  ...
  private static final String LOGTAG = "HttpServerModule";
  ...
  public void start(String _port, String root, Boolean localhost, Boolean keepAlive, Promise promise) {
    ...
    try {
      ...
      if(localhost_only) {
        server = new HttpServer("localhost", port, www_root);
      } else {
        server = new HttpServer(__getLocalIpAddress(), port, www_root);
      }
